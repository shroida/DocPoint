...
class MessageCubit extends Cubit<MessageState> {
  ...
  Future<void> makeMessagesRead(String userId) async {
    emit(MessageLoading());
    final result = await _makeMessagesRead(userId);
    result.fold(
      (Failure failure) => emit(MessageError(failure.message)),
      (_) {
        emit(MessageSent());
